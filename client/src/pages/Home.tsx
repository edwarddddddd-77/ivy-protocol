...
export default function Home() {
  ...
  const { dailyMintAmount, cbStatus, effectiveAlpha, ivyBalance, nodeTotalSupply, pidMultiplier, address, isLoading } = useIvyContract();

  // Circuit Breaker Status
  const isRedAlert = cbStatus?.isActive && cbStatus?.level === 3; // Level 3 = RED
  const pidValue = effectiveAlpha;

  // Navigate to /nodes instead of minting directly
        ...
        <div className="flex-1 flex flex-col items-start space-y-6 md:space-y-8 max-w-2xl z-30 w-full">
          ...
          >
            ...
            <h1 className="text-4xl sm:text-5xl md:text-8xl font-display font-bold uppercase leading-[0.9] tracking-tight text-[#E0E0E0] mix-blend-difference relative group cursor-default">
              <GlitchText text={t('home.title.line1')} /> <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-400">{t('home.title.line2')}</span>
            </h1>
            
            <p className="text-sm md:text-xl text-slate-400 font-mono max-w-lg border-l-2 border-[#39FF14]/30 pl-4">
              {t('home.subtitle')}
              <br />
              <span className="text-[#39FF14] text-xs md:text-sm opacity-80">{t('home.subtitle.highlight')}</span>
            </p>
          </motion.div>
          ...
          >
            ...
            >
              <span className="relative z-10 flex items-center gap-2">
                ...
                {isRedAlert ? t('home.cta.locked') : t('home.cta')}
              </span>
          ...
          {isLoading ? (
            <>
              <div className="flex flex-row md:flex-col gap-4 w-full md:w-auto">
                <StatsCardSkeleton className="flex-1 md:flex-none" />
                <StatsCardSkeleton className="flex-1 md:flex-none" />
              </div>
              <div className="flex flex-row md:flex-col gap-4 w-full md:w-auto">
                <StatsCardSkeleton className="flex-1 md:flex-none" />
